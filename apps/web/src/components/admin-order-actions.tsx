...
export function AdminOrderActions({ orderId, availableActions, refunds }: {
  ...
  return (
    <section className="checkout-card" aria-labelledby="order-actions-heading">
      ...
      {availableActions.includes("CANCEL") && <details className="mt-4 rounded-2xl border border-[var(--line)] p-4"><summary className="cursor-pointer text-sm font-medium text-(--wine)">Cancel order</summary><label className="mt-4 block text-sm">Cancellation reason<textarea className="admin-input mt-2 min-h-24" maxLength={500} minLength={5} onChange={(event) => setReason(event.target.value)} required value={reason} /></label><button className="mt-3 w-full rounded-full border border-[var(--wine)] px-5 py-3 text-sm font-semibold text-(--wine) disabled:opacity-50" disabled={Boolean(pending) || reason.trim().length < 5} onClick={() => mutate(`orders/${orderId}/cancellations`, { reason }, "CANCEL")} type="button">{pending === "CANCEL" ? "Cancelling safely…" : "Confirm cancellation"}</button></details>}
      {availableActions.includes("COMPLETE_REFUND") && pendingRefund && <div className="mt-4 rounded-2xl bg-[var(--sand)] p-4"><p className="text-sm font-medium">Refund due</p><p className="mt-1 text-xs leading-5 text-(--muted)">Send the refund outside the store, then record its transfer reference here.</p><label className="mt-3 block text-sm">Transfer reference <span className="text-(--muted)">(optional)</span><input className="admin-input mt-2" maxLength={120} onChange={(event) => setReference(event.target.value)} value={reference} /></label><button className="admin-primary mt-3 w-full" disabled={Boolean(pending)} onClick={() => mutate(`refunds/${pendingRefund.id}/complete`, { manual_reference: reference.trim() || null }, "COMPLETE_REFUND")} type="button">{pending === "COMPLETE_REFUND" ? "Recording refund…" : "Mark refund sent"}</button></div>}
      {!availableActions.length && <p className="mt-3 text-sm text-(--muted)">No further order actions are available.</p>}
