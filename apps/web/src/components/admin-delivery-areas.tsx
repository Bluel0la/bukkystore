  ...
  return (
    <div className="grid gap-5">
      ...
      <section className="checkout-card" aria-labelledby="areas-heading">
        ...
        <p className="mt-1 text-sm text-(--muted)">Checkout only offers active areas. Deactivate instead of deleting to keep order history intact.</p>
        <div className="mt-4 grid gap-3">
          {initial.map((area) => (
            <div className="flex flex-wrap items-center gap-3 rounded-2xl border border-[var(--line)] p-3" key={area.id}>
              <div className="min-w-36 flex-1">
                <p className="font-medium">{area.name}</p>
                <p className="mt-1 text-xs text-(--muted)">{formatNaira(area.fee_minor)}{area.is_active ? "" : " · inactive"}</p>
              </div>
          ...
          {!initial.length && <p className="text-sm text-(--muted)">No delivery areas yet. Add the first one above.</p>}
        </div>
