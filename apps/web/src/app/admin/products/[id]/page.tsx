...
export default async function EditProductPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="mx-auto min-h-screen max-w-5xl px-5 py-6 sm:px-8"><header className="mb-8 flex items-center justify-between"><div><p className="text-sm text-(--muted)">{product.category.name}</p><h1 className="text-3xl font-semibold tracking-[-0.04em]">{product.name}</h1></div><Link className="rounded-full border border-[var(--line)] px-4 py-2 text-sm" href="/admin/products">Back</Link></header><div className="grid gap-5"><AdminSharePanel engagement={engagement} productSlug={product.slug} /><AdminProductEditor categories={categories} product={product} /></div></main>;
}
