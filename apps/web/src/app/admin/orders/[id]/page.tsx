  ...
  return (
    <main className="mx-auto min-h-screen max-w-4xl px-5 py-6 sm:px-8">
      <header className="mb-8">
        <Link className="text-sm text-[var(--muted)] underline" href="/admin/orders">← All orders</Link>
        <div className="mt-4 flex flex-wrap items-end justify-between gap-4">
          <div><p className="text-sm text-[var(--wine)]">{order.status.replaceAll("_", " ")}</p><h1 className="mt-1 text-3xl font-semibold tracking-[-0.04em]">{order.order_number}</h1></div>
      ...
      <div className="grid gap-5 md:grid-cols-[1.35fr_1fr]">
        <div className="grid gap-5">
          <section className="checkout-card">
            ...
            <div className="mt-4">{order.items.map((item) => <div className="flex justify-between gap-4 border-t border-[var(--line)] py-4 first:border-0" key={item.id}><div><p className="font-medium">{item.product_name} × {item.quantity}</p><p className="mt-1 text-xs text-[var(--muted)]">{item.variant_name} · {item.sku}</p></div><span>{formatNaira(item.line_subtotal_minor)}</span></div>)}</div>
        ...
        <aside className="grid content-start gap-5">
          <section className="checkout-card">
            <h2 className="text-lg font-semibold">Customer</h2><p className="mt-4 font-medium">{order.customer_full_name}</p><p className="mt-1 text-sm text-[var(--muted)]">{order.customer_phone}</p>{order.customer_email && <p className="mt-1 break-all text-sm text-[var(--muted)]">{order.customer_email}</p>}
          ...
          <section className="checkout-card"><h2 className="text-lg font-semibold">Delivery</h2><p className="mt-4 text-sm font-medium">{order.delivery_area_name}</p><p className="mt-2 text-sm leading-6 text-[var(--muted)]">{order.delivery_address}</p>{order.delivery_directions && <p className="mt-3 border-t border-[var(--line)] pt-3 text-xs leading-5 text-[var(--muted)]">Landmark: {order.delivery_directions}</p>}</section>
          <section className="checkout-card"><h2 className="text-lg font-semibold">Payment</h2><p className="mt-4 text-sm">{order.payment_status.replaceAll("_", " ")}</p>{order.refunds.map((refund) => <div className="mt-3 rounded-xl bg-[var(--sand)] p-3 text-sm" key={refund.id}><p>Refund: {refund.status}</p><p className="mt-1 text-xs text-[var(--muted)]">{formatNaira(refund.amount_minor)}</p></div>)}</section>
        </aside>
      </div>
    </main>
  );
