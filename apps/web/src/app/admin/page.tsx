...
import { getAdminAnalytics, getAdminOrders, getAdminUser } from "@/lib/admin";
import { formatNaira } from "@/lib/catalogue";

export default async function AdminPage() {
  const [user, analytics, orderPage] = await Promise.all([
    getAdminUser(),
  ...
  const maxUnits = Math.max(...analytics.top_products.map((product) => product.units_sold), 1);
  const maxSourceOrders = Math.max(...analytics.sources.map((source) => source.orders), 1);
  const cards = [
    ...
    <main className="mx-auto min-h-screen max-w-6xl px-5 py-6 sm:px-8">
      <header className="mb-8 flex flex-wrap items-end justify-between gap-5">
        <div>
          ...
          <p className="mt-2 max-w-xl text-sm leading-6 text-[var(--muted)]">
            A current view of sales, fulfilment, refunds, and stock across Bukky Store.
          </p>
        </div>
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-[1.35fr_0.85fr]">
        ...
        <section aria-labelledby="stock-heading">
          ...
          <div className="mt-4 overflow-hidden rounded-2xl border border-[var(--line)] bg-white">
            {analytics.low_stock.map((variant) => (
            ...
            {!analytics.low_stock.length && <p className="p-8 text-center text-sm text-[var(--muted)]">Stock levels look healthy.</p>}
          </div>
        </section>
      </div>
      ...
      <div className="mt-8 grid gap-6 md:grid-cols-2">
        <section className="rounded-2xl border border-[var(--line)] bg-white p-5" aria-labelledby="products-heading">
          ...
          <div className="mt-6 grid gap-5">
            ...
            {!analytics.top_products.length && <p className="text-sm text-[var(--muted)]">Paid orders will reveal the strongest products.</p>}
          </div>
        </section>
        ...
        <section className="rounded-2xl border border-[var(--line)] bg-white p-5" aria-labelledby="sources-heading">
          ...
          <div className="mt-6 grid gap-5">
            ...
            {!analytics.sources.length && <p className="text-sm text-[var(--muted)]">Source results will appear after paid checkouts.</p>}
          </div>
        </section>
      </div>
    </main>
